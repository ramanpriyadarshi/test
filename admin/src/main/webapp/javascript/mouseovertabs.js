...
ajaxload:function(tabsmenuid, submenuid, disappearBool, url){
	...
	if (window.ActiveXObject){ //Test for support for ActiveXObject in IE first (as XMLHttpRequest in IE7 is broken)
		try {
			...
			//alert("after XMLHTTP -- " + page_request);
		} 
		catch (e){
			try{
				//alert("before Micro");
	...
	//alert("bustcache -- " + bustcache);
},

ajaxpopulate:function(page_request, tabsmenuid, submenuid, disappearBool, url){
	//alertthis("ajaxpopulate");
	//alertthis("page_request.readyState -- " + page_request.readyState);
	//alertthis("page_request.status -- " + page_request.status);
	//alertthis("window.location.href -- " + window.location.href);
	//alertthis("window.location.href.indexOf(\"http\") -- " + window.location.href.indexOf("http"));
	if (page_request.readyState == 4 && (page_request.status==200 || window.location.href.indexOf("http")==-1)){
		var tabsmenutree=this.tabsmenutree[tabsmenuid];
		//alertthis("tabsmenutree -- " + tabsmenutree);
		tabsmenutree.submenu.innerHTML=page_request.responseText
		var innerdivs=tabsmenutree.submenu.getElementsByTagName("div");
		//alertthis("innerdivs -- " + innerdivs);
		for (var i=0; i<innerdivs.length; i++){
			//if (/tabsmenucontent/i.test(innerdivs[i].className)){
			if (/links_left/i.test(innerdivs[i].className)){
...
init:function(tabsmenuid, submenuid, disappearBool){
	...
	//alert("submenu._parentid -- " + submenu._parentid);
...
function alertthis(text) {
	//temp = document.getElementById("alert").innerHTML;
	//document.getElementById("alert").innerHTML = temp + "|" + text;
